...
def clearPath(vehID, tlsLinkMap, usedTLS, j2tls, tlss):
    ...
    if edge in tlsLinkMap:
        ...
        #Distance to the tls
        tls2j= {"maschpl" : "3792043772", "celler": "59654713", "rudolfplatz": "1786940099", "kaelberwiese": "635103577", "maienstr": "3323411891", "kreuzstr": "5502963199", "madamenweg": "5502964928", "broitzemer": "5624016221", "broitzemer": "5749804898", "Cyriaksring": "59708186"}
        if tlid in tls2j:
            junction= tls2j[tlid]
        else:
            junction= tlid
...
def singleRun(run, evRoute):
	...
	#--step-length 0.1
    sumoCmd = [sumoExe, "-c", sumoConfig, "--tripinfo-output",  "stream_sirene\\tripinfos%s_%s.xml" %(run, evRoute),  "--random"]
    traci.start(sumoCmd) 
