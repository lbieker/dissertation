...
def calculateTrafficJam(vehID, edgeID, tlss, net):
    veh_number= 0
    #todo automatically
    ...
    nextEdgeIDs= net.getEdge(edgeID).getOutgoing()
    for id in nextEdgeIDs:
        vehEdge=traci.edge.getLastStepVehicleNumber(edgeID)
        vehNextEdge= traci.edge.getLastStepVehicleNumber(id.getID())
        edgeLength= 185.6 #should be done automatically
        max_distance2= -1
        if not hasCapacity(vehEdge, vehNextEdge, edgeLength):
            ...
            max_distances2[id.getID()]= max_distance2
            #print "No capacity"    
        # fuer extended approach
    return max_distance, max_distances2
...
def clearPath(vehID, tlsLinkMap, usedTLS, j2tls, tlss, net, tlsNotOnRoute):
    ...
    if edge in j2tls.keys():
        ...
        if edge in tlsNotOnRoute:# and tlsNotOnRoute[edge] in usedTLS :
            for e in tlsNotOnRoute[edge]:
                #print "aufloesen", tlsNotOnRoute[edge], edge
                programID= '0'
                #usedTLS.remove(tlsNotOnRoute[edge])
                try:
                    traci.trafficlight.setProgram(e , programID)   #j2tls[edge] 
                    #print "Set Traffic light", e , edge
                except:
                    print("Junction has no tls", usedTLS)
            del tlsNotOnRoute[edge]
    if edge in tlsLinkMap:
        ...
        max_distance2= -1
        if not edge==-1:
            max_distance, max_distance2= calculateTrafficJam(vehID, edge, tlss, net)
            #print max_distance
        if edge in tlsLinkMap:
            state= ''
        ...
        if (max_distance2 > -1):
            for id, max2 in max_distance2.items():
                #print max2
                state= ''
                route=  traci.vehicle.getRoute(vehID)
                current= route.index(edge)
                #look for all next edges
                nextEdgeID= id #route[current+1]
                if nextEdgeID in tlsLinkMap:
                    ...
                    if new_distance < max2:
                    #todo um liste erweitern 
                        if route[current+1] in tlsNotOnRoute:
                            tlsNotOnRoute[route[current+1]].append(tlsLinkMap[nextEdgeID][0])
                        else:
                            tlsNotOnRoute[route[current+1]]= [tlsLinkMap[nextEdgeID][0]]#tlid finden von edge 
                        #print tlsLinkMap[nextEdgeID],nextEdgeID
                        #print route[current+1], tlid2, tlsNotOnRoute[route[current+1]]
                        # set TLS Phase
                        #todo check for conflicts only
                        ...
                        print "Emergency TLS preemption for next TLS", tlsLinkMap[nextEdgeID]#tlid2
    return tlsNotOnRoute         
...
def singleRun(run, evRoute):
	...
	#--step-length 0.1
    sumoCmd = [sumoExe, "-c", sumoConfig, "--tripinfo-output",  "walabi_grid\\tripinfos%s_%s.xml" %(run, evRoute), "--random"]
    traci.start(sumoCmd) 
    ...
    while step < 3600: #traci.simulation.getMinExpectedNumber() > 0: 
        ...
        for vehID in emergencyIDs:
            if vehID in traci.simulation.getArrivedIDList():
                ...
                tlsNotOnRoute= clearPath(vehID, tlsLinkMap, usedTLS, j2tls, tlss, net, tlsNotOnRoute)
    print('TraCI Closed')    
    traci.close()
    shutil.copyfile("grid\\tls_state.xml", "walabi_grid\\tls_%s.xml" %(run))
