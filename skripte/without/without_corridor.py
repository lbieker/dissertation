...
def singleRun(run, evRoute):
    ...
    print sumoExe
    sumoConfig = "corridor\\run.sumocfg" #braunschweig_FK\\run.sumocfg"
	#--step-length 0.1
    sumoCmd = [sumoExe, "-c", sumoConfig, "--tripinfo-output",  "without_corridor\\tripinfos%s_%s.xml" %(run, evRoute), "--random"]
    traci.start(sumoCmd) 
    ...
    print('TraCI Closed')    
    traci.close()
    shutil.copyfile("corridor\\tls_state.xml", "without_corridor\\tls_%s.xml" %(run))
