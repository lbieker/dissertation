...
def clearPath(vehID, tlsLinkMap, usedTLS):
    ...
    position2= traci.junction.getPosition("0")#"C") 
...
def singleRun(run, evRoute):
    ...
    print sumoExe
    sumoConfig = "very_simple_intersection\\scenario\\run.sumocfg  " #braunschweig_FK\\run.sumocfg"
	#--step-length 0.1
    sumoCmd = [sumoExe, "-c", sumoConfig, "--tripinfo-output",  "fast_results\\tripinfos_%s_%s.xml" %(run, evRoute),  "--random"]
    traci.start(sumoCmd) 
    ...
    print('TraCI Closed')    
    traci.close()
    shutil.copyfile("very_simple_intersection\\scenario\\tls_state.xml", "fast_results\\tls_%s.xml" %(run))
