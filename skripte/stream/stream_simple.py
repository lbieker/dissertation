...
def singleRun(run, evRoute):
	...
	#--step-length 0.1
    sumoCmd = [sumoExe, "-c", sumoConfig, "--tripinfo-output",  "stream\\tripinfos_%s_%s.xml" %(run, evRoute),  "--random"]
    traci.start(sumoCmd) 
    ...
    print('TraCI Closed')    
    traci.close()
    shutil.copyfile("very_simple_intersection\\scenario\\tls_state.xml", "stream\\tls_%s.xml" %(run))
