...
def singleRun(run, evRoute):
	...
	#--step-length 0.1
    sumoCmd = [sumoExe, "-c", sumoConfig, "--tripinfo-output",  "walabi_results_fk\\tripinfos_%s_%s.xml" %(run, evRoute),  "--random"]
    traci.start(sumoCmd) 
    ...
    print('TraCI Closed')    
    traci.close()
    shutil.copyfile("braunschweig_FK\\tls_state.xml", "walabi_results_fk\\tls_%s.xml" %(run))
