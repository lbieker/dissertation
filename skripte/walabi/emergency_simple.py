...
def calculateTrafficJam(edgeID):
    veh_number= 0
    #todo automatically
    edge2lane= {'1i': 1, '2i':1, '3i': 1, '4i': 1}#{'166445405#0': 5, 'e1': 2, 's1': 2, '61734682#3.75': 3, '61734682#3.115': 5, '318881710#0': 3, 'n1': 2, '314915850#0': 5, '314915843': 4, 'w1': 3}
    ...
    if edgeID in edge2lane.keys():
        ...
        for id in range(edge2lane[edgeID]):
            laneID= edgeID + '_' + str(id) # pedestrian and bike way are excluded
            lanes.append(laneID)
        for laneID in lanes:
            num_lane= 0  
            try: 
...
def clearPath(vehID, tlsLinkMap, usedTLS):
    ...
    position2= traci.junction.getPosition("0") 
...
def singleRun(run, evRoute):
    ...
    print sumoExe
    sumoConfig = "very_simple_intersection\\scenario\\run.sumocfg"
	#--step-length 0.1
    sumoCmd = [sumoExe, "-c", sumoConfig, "--tripinfo-output",  "walabi\\tripinfos_%s_%s.xml" %(run, evRoute),  "--random"]
    traci.start(sumoCmd) 
    ...
    print('TraCI Closed')    
    traci.close()
    shutil.copyfile("very_simple_intersection\\scenario\\tls_state.xml", "walabi\\tls_%s.xml" %(run))
