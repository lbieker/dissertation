...
def singleRun(run, evRoute):
	...
	#--step-length 0.1
    sumoCmd = [sumoExe, "-c", sumoConfig, "--tripinfo-output",  "stream_grid\\tripinfos%s_%s.xml" %(run, evRoute), "--random"]
    traci.start(sumoCmd) 
    ...
    print('TraCI Closed')    
    traci.close()
    shutil.copyfile("grid\\tls_state.xml", "stream_grid\\tls_%s.xml" %(run))
