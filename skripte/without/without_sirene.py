...
def singleRun(run, evRoute):
    ...
    print sumoExe
    sumoConfig = "SUMO_Sirene\\sirene.sumocfg" #braunschweig_FK\\run.sumocfg"
	#--step-length 0.1
    sumoCmd = [sumoExe, "-c", sumoConfig, "--tripinfo-output",  "without_sirene\\tripinfos_%s_%s.xml" %(run, evRoute), "--random"]
    traci.start(sumoCmd) 
