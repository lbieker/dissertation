...
def singleRun(run, evRoute):
    net = sumolib.net.readNet("corridor\\corridor.net.xml") #sumonet.NetReader()
    ...
    print sumoExe
    sumoConfig = "corridor\\run.sumocfg"
	#--step-length 0.1
    sumoCmd = [sumoExe, "-c", sumoConfig, "--tripinfo-output",  "stream_corridor\\tripinfos%s_%s.xml" %(run, evRoute),  "--random"]
    traci.start(sumoCmd) 
