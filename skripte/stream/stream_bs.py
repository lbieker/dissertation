...
def calculateTrafficJam(edgeID, tlss):
    veh_number= 0
    #todo automatically
...
def clearPath(vehID, tlsLinkMap, usedTLS, j2tls, tlss):
    ...
    if edge in tlsLinkMap:
        ...
        #Distance to the tls
        tls2j= {"a1": "54532608", "a3": "54532540", "f1":"54536753", "d9": "54535620", "a2": "54535263", "a5": "54535257", "a6": "54535186", "f3": "54531922", "d5": "54542095", "221": "54534889", "e5": "54533980", "b3a": "54533985", "d3": "54534192", "b4a": "54534970", "c2": "54534963", "c1": "54534984", "a6": "54534945"}#tls2j= {"maschpl" : "3792043772", "celler": "59654713", "rudolfplatz": "1786940099", "kaelberwiese": "635103577", "maienstr": "3323411891", "kreuzstr": "5502963199", "madamenweg": "5502964928", "broitzemer": "5624016221", "broitzemer": "5749804898", "Cyriaksring": "59708186"}
        if tlid in tls2j:
            junction= tls2j[tlid]
        else:
            junction= tlid
        ...
        max_distance= 0
        if not edge==-1:
            max_distance= 100#calculateTrafficJam(edge, tlss)
            #print max_distance
        if edge in tlsLinkMap:
            state= ''
            ...
            if tlid not in usedTLS and distance < max_distance:
                ...
                #print len(traci.trafficlight.getRedYellowGreenState(tlid)), len(state)
...
def singleRun(run, evRoute):
    ...
    sumoCmd = [sumoExe, "-c", sumoConfig, "--tripinfo-output",  "stream_bs\\tripinfos%s_%s.xml" %(run, evRoute),  "--random"]
    traci.start(sumoCmd) 
    ...
    print('TraCI Closed')    
    traci.close()
    #shutil.copyfile("corridor\\tls_state.xml", "stream_corridor\\tls_%s.xml" %(run))
